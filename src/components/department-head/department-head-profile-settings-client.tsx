...
import { dhFormControlClass } from "@/components/department-head/department-head-ui-styles";
import {
  PROFILE_IMAGE_FILE_INPUT_ACCEPT,
  validateProfileImageFile,
} from "@/lib/profile-image-upload-rules";

type DepartmentHeadProfileData = {
  id: string;
  email: string;
  role: string;
  name: string;
...
function dispatchProfileUpdated(detail: { name?: string; profilePictureUrl?: string | null }) {
  window.dispatchEvent(new CustomEvent("department-head-profile-updated", { detail }));
}

export function DepartmentHeadProfileSettingsClient({
  initialProfile,
}: {
  initialProfile: DepartmentHeadProfileData;
}) {
  ...
  async function saveProfileName(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/department-head/profile", {
        method: "PATCH",
        headers: {
  ...
  async function uploadProfilePicture() {
    ...
    try {
      ...
      const response = await fetch("/api/department-head/profile-picture", {
        method: "POST",
        body: payload,
      });
  ...
  return (
    <div className="space-y-4">
      <SectionCard title="Profile Picture" description="Upload your profile photo for top-bar identity.">
        <div className="flex flex-col gap-4 md:flex-row md:items-start">
          ...
          <div className="min-w-0 flex-1 space-y-3">
            <input
              ...
              className={`block w-full max-w-md rounded-xl border border-[var(--border-color)] bg-[var(--surface)] px-3 py-2 text-sm ${dhFormControlClass}`}
            />
      ...
      <SectionCard title="Account Details" description="Signed-in Department Head identity. Email, role, and user ID are read-only.">
        <AccountDetailsPanel
          items={[
            {
              label: "Display Name",
      ...
      <SectionCard title="Name Setup" description="Update your personal display name.">
